...
> 日常开发流程（改完跑哪条命令、怎么提交、文档跟着谁改）见 [本目录的 AGENTS.md](AGENTS.md)，
...
种子会建 1 位主讲老师（沈老师）+ 5 位 AI 同学（林晓 / 陈默 / 苏雨 / 周野 / 顾棠，课堂按设置里的「AI 同学数量」取前 N 位）+ 3 个音色 + **2 门官方示例课**（机器学习入门 / 光合作用，各 12 页）；重复执行不会产生重复数据，也不会覆盖你改过的行。
