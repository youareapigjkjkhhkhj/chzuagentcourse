...
MESSAGES = {
    ...
    40201: "未配置 API Key",
    # 40302 是部署决定不是故障（P2-G3）：关掉语音后课堂仍是可用的纯文字课堂
    40302: "语音功能已关闭",
    # 40303 同理（P4-G3）：关掉材料后还能凭主题生成，只是不能带资料进来
    40303: "材料功能已关闭",
    # 40304 是部署配置（P5-A13）：开了站点访问码而这次请求没带。
    # 前端见到它就跳 `/access`，不弹错误框 —— 那不是故障，是还没进门。
    40304: "需要访问码",
    42901: "请求过于频繁，请稍后重试",
    50001: "服务器内部错误",
    # 50002 是**这台部署**没配好加密密钥（FERNET_KEY），不是代码炸了：
    # 文案由 CryptoError 自带（说清怎么生成、填哪儿、要不要重启），这里是兜底。
    # 它得是 AppError，否则会掉进「未知异常」那一格，用户只看到 50001。
    50002: "服务端未配置加密密钥（FERNET_KEY）",
    50201: "上游服务调用失败",
    50401: "上游服务响应超时",
}
