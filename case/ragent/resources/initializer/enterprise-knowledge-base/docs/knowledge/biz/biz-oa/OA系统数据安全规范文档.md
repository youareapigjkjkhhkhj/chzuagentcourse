...
* 一般访问日志 ≥ 180 天；
* 管理员操作、财务凭证与权限变更类日志 ≥ 3 年；
* L4 数据（薪酬要素、银行账户、证件影像）的查看、下载与导出日志 ≥ 5 年；
* 审计系统只增不改，支持导出可校验的证据链。

**泄露追责依据**

高敏数据外泄时按以下证据链认定责任主体：文件水印还原操作账号、时间、来源单号与校验码 → 审计日志比对该账号的查看、下载、导出记录与其授权范围 → 行为基线判定是否属于异常访问模式 → 网关与主机日志还原传播路径。

责任认定后移交 HR 按《公司规章制度》违纪处理流程处置，涉及薪酬、银行账户、证件影像等 L4 数据外泄的按重大违纪处理，情节严重的解除劳动合同并保留追究法律责任的权利。
...
* **RPO（恢复点目标，Recovery Point Objective）**：故障恢复后允许丢失的数据时间窗口上限，即最坏情况下只能恢复到故障发生前 RPO 时长的数据状态，该时点之后写入的数据视为丢失。
* **RTO（恢复时间目标，Recovery Time Objective）**：从故障发生到业务服务重新可用之间允许经过的最长时间，是对停服时长的上限约束。
* 两项指标均自**故障发生时刻**起算，而非自故障被发现或告警触发时刻起算。
* OA 系统当前生效指标：**RPO ≤ 10 分钟、RTO ≤ 2 小时**。

**策略**

* 日全量、2 小时增量、周冷备、月跨地域；
* 事务日志（binlog/redo）持续归档至异地对象存储，归档间隔 ≤ 5 分钟，用于支撑 RPO ≤ 10 分钟的恢复精度；
* 备份加密、独立账号访问、定期恢复演练。
...
| 角色  | 本部门数据 | 下属部门 | 高敏字段 | 高敏附件（含工资单、证件影像） | 高敏导出   | 管理操作      |
| --- | ----- | ---- | ---- | --------------- | ------ | --------- |
| 员工  | ✔（本人） | ✖    | ✖/模糊 | 仅本人，水印预览        | ✖      | ✖         |
| 主管  | ✔     | ✔    | 模糊   | 薪酬类不可见；其他水印预览   | 申请后视审批 | ✖         |
| HR  | ✔     | ✔    | ✔    | 水印预览，下载需二审      | 二审+限流  | ✖         |
| 财务  | 相关流程  | 相关流程 | 金额可见 | 相关流程内水印预览，下载需二审 | 二审+限流  | ✖         |
| 管理员 | 依职责   | 依职责  | 依职责  | 依职责，水印预览，下载需二审  | 二审+限流  | 部分 ✔（强审计） |

**工资单附件的角色边界**

* 员工只能查看本人工资单附件，以水印方式预览，不提供下载。
* 主管**不能查看下属的工资单附件与薪酬明细**，只能在审批场景中看到区间值。带团队不等于可见下属薪酬。
* HR 可查看职责范围内的工资单附件，默认水印预览；下载须提交申请说明用途、范围与保存期限，经二级审批后放行，下载链接一次一签一次一用，并按量限流。
* 上述所有查看与下载动作全量写入审计日志，日志保留 ≥ 5 年。
