...
package com.example.ai.ragent.framework.distributedid;
...
/**
 * 分布�?Snowflake 初始化器
 * �?Redis 获取 workerId �?datacenterId，并注册�?Hutool �?IdUtil �? */
@Slf4j
@Component
...
public class SnowflakeIdInitializer {
    ...
    public void init() {
        ...
        try {
            // 执行 Lua 脚本获取 workerId �?datacenterId
            ...
            log.info("分布式Snowflake初始化完�? workerId: {}, datacenterId: {}", workerId, datacenterId);
        } catch (Exception e) {
            throw new RuntimeException("分布式Snowflake初始化失�?, e);
        }
    }
}
