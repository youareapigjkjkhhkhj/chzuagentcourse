...
public interface MessageQueueProducer {
     ...
     * @param bizDesc 业务描述，用于日志标�?     * @param body    业务载荷
     * @return RocketMQ 发送结果，包含 msgId、sendStatus 等信�?     */
    SendResult send(String topic, String keys, String bizDesc, Object body);

    /**
     * 发送事务消�?     * <p>
     * 流程：发�?half 消息 �?执行本地事务 �?根据结果 commit/rollback
     * <p>
     * 事务回查由按 topic 注册�?{@link TransactionChecker} 处理，需提前通过
     ...
     * @param bizDesc          业务描述
     * @param body             业务载荷
     * @param localTransaction 本地事务逻辑，在 half 消息发送成功后执行；抛异常则回滚消�?     */
