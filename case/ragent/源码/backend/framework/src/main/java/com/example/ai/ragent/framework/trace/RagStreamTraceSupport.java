...
package com.example.ai.ragent.framework.trace;

/**
 * 跨线�?stream 节点�?Trace 支持
 * <p>
 * 解决 @RagTraceNode AOP �?stream 场景下只测到任务提交（runAsync）的问题�? * 同步部分（HTTP 提交、首包阻塞）在调用线程执行，真正�?SSE 读循环在线程�? * worker 线程执行。本接口允许调用线程开启一个跨线程节点，由 SSE �? * onComplete / onError 在另一线程�?finish
 */
public interface RagStreamTraceSupport {

    /**
     * 在调用线程开启一�?stream 节点：插�?RUNNING 行，�?nodeId push 进当�?     * 线程�?NODE_STACK 以便嵌套同步子节点（�?first-packet）能识别父节�?     * <p>
     * 注意：返回的 {@link StreamSpan} 必须在调用线程同步部分结束前调用
     * {@link StreamSpan#detach()} �?nodeId 弹出 stack；finish 在异步线程触�?     */
    StreamSpan beginStreamNode(String name, String type);

    interface StreamSpan {

        /**
         * 调用线程同步部分结束时调用，�?nodeId �?NODE_STACK 弹出
         * 不会 finish 节点
         */
        void detach();
