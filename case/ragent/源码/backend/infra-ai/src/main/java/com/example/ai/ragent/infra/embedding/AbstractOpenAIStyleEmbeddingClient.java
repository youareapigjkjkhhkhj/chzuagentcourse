 ...
 * 封装 /v1/embeddings 协议的通用逻辑，子类只需提供 provider 和覆写钩子方�? */
@Slf4j
public abstract class AbstractOpenAIStyleEmbeddingClient implements EmbeddingClient {
    ...
    /**
     * 子类可覆写此方法添加提供商特有的请求体字�?     * 默认实现：添�?encoding_format=float
     */
    ...
    protected List<List<Float>> doEmbed(List<String> texts, ModelTarget target) {
        ...
        // embedding 是检索链路上唯一的远端同步调用，慢在这里会被上游按「通道无结果」吞�?        // 成败两路都记耗时：失败路更要记，慢失败才是吃满上游预算的那一�?        long startTime = System.currentTimeMillis();
        ...
        if (data == null || data.isEmpty()) {
            throw new ModelClientException(
                    provider() + " embedding 响应中缺�?data 数组",
        ...
        for (JsonElement el : data) {
            ...
            if (emb == null || emb.isEmpty()) {
                throw new ModelClientException(
                        provider() + " embedding 响应中缺�?embedding 字段",
                        ModelClientErrorType.INVALID_RESPONSE, null);
            }
