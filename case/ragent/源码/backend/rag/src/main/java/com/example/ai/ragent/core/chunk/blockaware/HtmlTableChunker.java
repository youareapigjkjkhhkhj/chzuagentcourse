...
/**
 * HTML 表格 chunker：按 tr 边界切分，每块重复表头行并包回完�?table
 * <p>
 * 不转成管道表：合并单元格与单元格内的换行在展开成二维表时会失真，展示与检索都用同一�?HTML
 */
@Component
public class HtmlTableChunker implements BlockChunker<HtmlTableBlock> {
    ...
    /**
     * 值为 1 �?colspan / rowspan 不表达任何合并，MinerU 却逐格都写，一张十来行的表能被它撑到三�?     */
    ...
    public List<ChunkDraft> chunk(HtmlTableBlock block, ChunkContext ctx) {
        ...
        // 整张表撑得住容忍上限就不切，切开后每块虽重带表头，跨块的行间对比仍然做不�?        int budget = rows.size() - 1 <= maxRows && html.length() <= ctx.budget().toleranceChars()
                ? ctx.budget().toleranceChars()
                : Math.max(1, ctx.budget().maxChars());
        // 外壳与表头每块都要重复，先从预算里扣掉，否则渲染出来必然�?        int overhead = open.length() + TABLE_CLOSE.length() + header.length();
    ...
    /**
     * 取原�?table 开标签而非写死，作者写在上面的 border / class 等属性得跟着每一块走
     */
