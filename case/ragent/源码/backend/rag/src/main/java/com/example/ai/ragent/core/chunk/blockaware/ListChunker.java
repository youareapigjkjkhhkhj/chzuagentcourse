...
/**
 * 列表 chunker：按渲染后的字符体量把列表项贪心分组，绝不从项中间切�? * <p>
 * 度量取字符而非项数：项数是体量的坏代理，十几条词条的清单合起来两百来字，按项数切开后打包器又原样并回来�? * 只在正文里留下一串多余空�? */
@Component
public class ListChunker implements BlockChunker<ListBlock> {
    ...
    public List<ChunkDraft> chunk(ListBlock block, ChunkContext ctx) {
        ...
        // 整份清单撑得住容忍上限就不切，切开后「要交哪些材料」这类问题只能召回半�?        int budget = Math.max(1, renderedLength(block) <= ctx.budget().toleranceChars()
