...
/**
 * 表格 chunker：按 key-value 渲染长度累加到预算，每块都带完整表头，单行超预算时整行原子成�? * <p>
 * {@code rowsPerChunk} 只作硬上限，兼顾宽表不超嵌入上限、窄表不过度碎片化；展示文本是完�? * markdown 表格，向量文本改�?{@code 列名: 值}，因�?markdown 表格靠位置对齐列名与值、嵌入模�? * 读不懂位置；表头不拼进向量文本，KV 正文已逐格自带列名，重复前缀会把同一张表各块的向量朝同一
...
public class TableChunker implements BlockChunker<TableBlock> {
    ...
    public List<ChunkDraft> chunk(TableBlock block, ChunkContext ctx) {
        ...
        // 预算只量 KV 行本身，刻意不扣装配器追加的章节路径前缀：真去扣，深层章节会把可用预算逼近 0�?        // 退化成每行一块、每块大半是逐字相同的前缀
        int maxRows = Math.max(1, ctx.budget().rowsPerChunk());
        // 整张表撑得住容忍上限就不切，切开后每块虽重带表头，跨块的行间对比仍然做不�?        int budget = rows.size() <= maxRows
    ...
    private ChunkDraft buildDraft(TableBlock block, ChunkContext ctx, List<String> headers, List<List<String>> rows) {
        ChunkMetadata metadata = ChunkMetadata.builder()
                ...
                .build();
        // 章节路径由装配器统一拼进向量文本，此处只�?key-value 正文，避免重复前缀
    ...
    /**
     * 单行渲染�?{@code 列名: 值}�?; " 拼接、跳过空值、整行空返回空串；同时用作预算切分的行体量度�?     */
    ...
    /**
     * �?cell 内换行压成空格：key �?value 之间夹一个断行会影响检�?     */
