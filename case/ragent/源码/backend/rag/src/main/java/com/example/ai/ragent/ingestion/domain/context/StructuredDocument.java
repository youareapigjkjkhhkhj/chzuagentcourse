 ...
 * 结构化文档实体类
 * <p>
 * 表示经过解析后的结构化文档，包含文档的纯文本内容、章节结构、表格以及元数据等信�? * 通过结构化解析，可以更好地保留文档的原始结构信息
 * </p>
 */
@Data
@NoArgsConstructor
...
public class StructuredDocument {
    ...
    /**
     * 结构�?Block 列表（v1.1 多模态解析改造引入）
     * <p>
     * �?DocumentParser.parseStructured() 输出，作�?ChunkerNode 的输入�?     * 老字�?{@link #sections}/{@link #tables} 仍保留以维持向后兼容�?     * 但新链路（M5/P1.7 之后）以 blocks 为准�?     */
    ...
    /**
     * 文档表格结构
     * 表示文档中的一个表格，包含表格标题、行数据以及在原文中的位置信�?     */
    @Data
    @NoArgsConstructor
    @AllArgsConstructor
    @Builder
