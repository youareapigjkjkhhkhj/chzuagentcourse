...
import com.example.ai.ragent.ingestion.domain.context.NodeLog;
...
public class IngestionEngine {
    ...
    public IngestionContext execute(PipelineDefinition pipeline, IngestionContext context) {
        ...
        // 验证流水线配�?        validatePipeline(nodeConfigMap);
        ...
        if (startNodeIds.size() > 1) {
            throw new ClientException("流水线存在多个起始节�? " + String.join(", ", startNodeIds));
        }
        ...
        log.info("流水线从节点开始执�? {}", startNodeId);

        // 从起始节点开始链式执�?        executeChain(startNodeId, nodeConfigMap, context);
            ...
            while (current != null) {
                ...
                if (StringUtils.hasText(nextId)) {
                    if (!nodeConfigMap.containsKey(nextId)) {
                        throw new ClientException("找不到下一个节�? " + nextId + "，被节点 " + current + " 引用");
                    }
                    current = nextId;
                } else {
                    break;
    ...
    private void executeChain(
        ...
        while (currentNodeId != null) {
            ...
            if (executedCount++ > maxNodes) {
                throw new ClientException("执行节点数超过上限，可能存在死循�?);
            }

            NodeConfig config = nodeConfigMap.get(currentNodeId);
            if (config == null) {
                log.warn("未找到节点配�? {}", currentNodeId);
                break;
            }

            log.info("开始执行节�? {}", currentNodeId);
            ...
            // 移动到下一个节�?            currentNodeId = config.getNextNodeId();
        }

        log.info("流水线执行完成，共执�?{} 个节�?, executedCount);
    }
    ...
    private NodeResult executeNode(IngestionContext context, NodeConfig nodeConfig) {
        ...
        if (node == null) {
            return NodeResult.fail(new IllegalStateException("未找到节点类�? " + nodeType));
        }

        // 条件检�?        if (nodeConfig.getCondition() != null && !nodeConfig.getCondition().isNull()) {
            if (!conditionEvaluator.evaluate(context, nodeConfig.getCondition())) {
                NodeResult skip = NodeResult.skip("条件未满�?);
