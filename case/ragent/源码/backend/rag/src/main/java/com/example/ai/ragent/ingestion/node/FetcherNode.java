...
import com.example.ai.ragent.ingestion.strategy.fetcher.DocumentFetcher;
import com.example.ai.ragent.ingestion.strategy.fetcher.FetchResult;
...
/**
 * 文档获取节点 (Fetcher Node)
 * 数据摄取负责从多元化的存储介质（�?Local FS、HTTP/HTTPS、OSS 等）中检索并载入文档原始字节�? * 核心逻辑采用策略模式 (Strategy Pattern) 实现，根�?{@link SourceType} 动态路由至具体�?{@link DocumentFetcher}
 * 具备幂等性检查机制：若上下文中已预置原始字节，则自动跳过获取流程，避免重�?I/O
 */
@Component
public class FetcherNode implements IngestionNode {
    ...
    public NodeResult execute(IngestionContext context, NodeConfig config) {
        ...
        return NodeResult.ok("已获�?" + (result.content() == null ? 0 : result.content().length) + " 字节");
    }
}
