...
package com.example.ai.ragent.knowledge.filter;

import com.example.ai.ragent.knowledge.config.RagSemaphoreProperties;
...
public class UploadRateLimitFilter extends OncePerRequestFilter {
    ...
    protected void doFilterInternal(@NonNull HttpServletRequest request,
        ...
        // 判断是否是上传请�?        if (!isUploadRequest(request)) {
            chain.doFilter(request, response);
            return;
        }

        // 获取信号量配�?        RagSemaphoreProperties.PermitExpirableConfig config = semaphoreProperties.getDocumentUpload();
        ...
        try {
            ...
            if (permitId == null) {
                ...
                return; // 不调�?chain.doFilter()，请求到此为�?            }

            // 获取许可成功，继续处理请�?            chain.doFilter(request, response);
