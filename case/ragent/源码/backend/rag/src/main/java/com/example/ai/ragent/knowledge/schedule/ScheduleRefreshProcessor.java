...
import com.example.ai.ragent.knowledge.dao.entity.KnowledgeDocumentScheduleDO;
import com.example.ai.ragent.knowledge.dao.entity.KnowledgeDocumentScheduleExecDO;
...
import com.example.ai.ragent.knowledge.dao.mapper.KnowledgeDocumentScheduleExecMapper;
import com.example.ai.ragent.knowledge.dao.mapper.KnowledgeDocumentScheduleMapper;
import com.example.ai.ragent.knowledge.enums.DocumentStatus;
import com.example.ai.ragent.knowledge.enums.ScheduleRunStatus;
import com.example.ai.ragent.knowledge.enums.SourceType;
import com.example.ai.ragent.knowledge.handler.RemoteFileFetcher;
import com.example.ai.ragent.knowledge.service.impl.KnowledgeDocumentServiceImpl;
...
public class ScheduleRefreshProcessor {
    ...
    public void process(ScheduleLockLease lease) {
        ...
        try {
            ...
            if (!released) {
                log.warn("定时刷新启动锁心跳失败且释放锁失�? scheduleId={}, lockToken={}",
                        lease.scheduleId(), lease.lockToken());
            }
            return;
        }
        ...
        try {
            ...
            if (state.document == null || (state.document.getDeleted() != null && state.document.getDeleted() == 1)) {
                disableIfOwnedOrMarkLeaseLost(lease, state, "文档不存在或已删�?, "禁用调度: 文档不存在或已删�?);
                return;
            }
            if (state.document.getEnabled() != null && state.document.getEnabled() == 0) {
                disableIfOwnedOrMarkLeaseLost(lease, state, "文档已禁�?, "禁用调度: 文档已禁�?);
                return;
            }
            ...
            if (enabled) {
                ...
                disableIfOwnedOrMarkLeaseLost(lease, state, "定时已关�?, "禁用调度: 定时已关�?);
                return;
            }
            ...
            try (RemoteFileFetcher.RemoteFetchResult fetchResult = remoteFileFetcher.fetchIfChanged(
                ...
                if (!fetchResult.changed()) {
                    markSkippedIfOwnedOrMarkLeaseLost(lease, state, fetchResult, "远程文件未变�?);
                    return;
                }
                ...
                if (shouldAbortForLeaseLoss(lease, heartbeat, "领取文档运行�?)) {
                    state.leaseLost = true;
                    stateManager.markLeaseLost(state.ctx, "领取文档运行�?);
                    return;
                }
                if (!documentStatusHelper.tryMarkRunning(state.document.getId())) {
                    markSkippedIfOwnedOrMarkLeaseLost(lease, state, "文档正在分块中，跳过本次调度", "文档运行权争抢失�?);
                    return;
                }
                ...
                if (latest == null || !DocumentStatus.SUCCESS.getCode().equals(latest.getStatus())) {
                    markFailedIfOwnedOrMarkLeaseLost(lease, state, "分块失败", "分块失败写回调度状�?);
                    return;
                }
                ...
                markSuccessIfOwnedOrMarkLeaseLost(lease, state, fetchResult, "刷新成功写回调度状�?);
            }
        } catch (Exception e) {
            ...
            if (state.phase != Phase.FILE_SWITCHED) {
                ...
                if (state.ctx != null) {
                    markFailedIfOwnedOrMarkLeaseLost(lease, state, e.getMessage(), "异常失败写回调度状�?);
                }
                ...
                log.error("定时刷新已完成文档切换，但写回调度状态失�? scheduleId={}, lockToken={}",
                        lease.scheduleId(), lease.lockToken(), e);
            }
        } finally {
            heartbeat.close();
    ...
    private boolean shouldAbortForLeaseLoss(ScheduleLockLease lease,
        ...
        if (heartbeat != null && heartbeat.isLost()) {
            log.warn("定时刷新锁已丢失，停止继续执�? scheduleId={}, stage={}, lockToken={}",
    ...
    private void logScheduleStateWriteSkipped(ScheduleLockLease lease, String stage) {
        log.warn("定时刷新锁已失效，未写回调度主状�? scheduleId={}, stage={}, lockToken={}",
                lease.scheduleId(), stage, lease.lockToken());
    }
