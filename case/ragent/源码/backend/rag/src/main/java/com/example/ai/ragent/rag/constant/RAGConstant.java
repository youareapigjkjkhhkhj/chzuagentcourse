...
package com.example.ai.ragent.rag.constant;
 ...
 *
 * <p>
 * 这些常量主要用于控制 RAG 系统的行为和生成质量，包括意图过滤、查询优化�? * 文档检索和智能问答等核心流�? * </p>
 */
public class RAGConstant {

    /**
     * 意图识别最低分数阈�?     * <p>
     * 低于这个分数就当�?聊偏�?，不参与 RAG 检索流�?     * </p>
     */
    ...
    /**
     * 多通道检索占位符�?     * <p>
     * 当没有意图识别结果时，使用此键作�?intentChunks Map 的占位符
     * 实际处理时只使用 Map �?values，不关心具体�?key �?     * </p>
     */
    public static final String MULTI_CHANNEL_KEY = "multi_channel";

    /**
     * 意图识别提示词模板路径（串行模式�?     * 一次性发送所有意图节点给 LLM 进行识别
     */
    ...
    /**
     * 歧义确认提示词模板路�?     * 用于边界 case 时调 LLM 二次确认是否存在品类歧义
     */
    ...
    /**
     * 会话标题生成提示词模板路�?     * 通过 {@code {title_max_chars}} �?{@code {question}} 控制标题长度与输入问�?     */
    public static final String CONVERSATION_TITLE_PROMPT_PATH = "prompt/conversation-title.st";


    /**
     * 知识资料回答的行内引用规�?     * <p>
     * 仅在 {@code rag.citation.enabled=true} 且存在知识库上下文时，由 Prompt 编排层追�?     */
    public static final String ANSWER_CITATION_RULES_PROMPT_PATH = "prompt/answer-citation-rules.st";

    /**
     * MCP 工具参数提取提示词模板路�?     * 用于从用户问题中提取工具调用参数
     */
    public static final String MCP_PARAMETER_EXTRACT_PROMPT_PATH = "prompt/mcp-parameter-extract.st";

    /**
     * MCP 工具参数提取用户消息提示词模板路�?     * 用于构建包含工具定义和用户问题的用户消息，通过 {@code {tool_definition}} �?{@code {user_question}} 占位符注入内�?     */
    public static final String MCP_PARAMETER_EXTRACT_USER_PROMPT_PATH = "prompt/mcp-parameter-extract-user.st";




    // ==================== 上下文格式化模板（单文件�?section�?====================

    /**
     * 上下文格式化模板文件路径
     * <p>
     * 包含所有上下文格式化所需�?section，通过 {@code --- section: name ---} 分隔�?     * 使用 {@code PromptTemplateLoader.renderSection(path, section, slots)} 渲染
     */
