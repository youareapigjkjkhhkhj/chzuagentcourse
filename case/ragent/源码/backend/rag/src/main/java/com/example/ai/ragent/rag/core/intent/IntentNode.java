...
public class IntentNode {
    ...
    /**
     * 唯一标识，如�?     * - "group" / "group-hr" / "biz-oa-intro" / "middleware-redis"
     */
    private String id;
    ...
    /**
     * 示例问题：尤其是“叶子节点”，可以放典型问法，帮助向量模型更精准对�?     */
    @Builder.Default
    ...
    /**
     * Milvus Collection 名称（仅�?kind=KB 有意义）
     * 仅用于兼容旧缓存和旧数据
     */
    private String collectionName;

    /**
     * 一�?KB 意图可关联多个逻辑 Collection
     */
    @Builder.Default
    private List<String> collectionNames = new ArrayList<>();

    /**
     * MCP 工具 ID（仅�?kind=MCP 有意义）
     */
    ...
    /**
     * 参数提取提示词模板（MCP 模式专属�?     * 如果配置了此字段，MCP 参数提取时使用自定义提示�?     */
    private String paramPromptTemplate;

    /**
     * 是否为“最终节点”（叶子节点）：
     * - 叶子节点才挂知识库（Milvus Collection�?     * - 叶子节点才会参与意图匹配打分
     */
     ...
     * 新字段优先，旧的�?Collection 字段仅作平滑升级兜底
     */
    @JsonIgnore
