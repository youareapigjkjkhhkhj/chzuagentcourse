...
import com.example.ai.ragent.rag.dto.IntentCandidate;
...
import static com.example.ai.ragent.rag.constant.RAGConstant.INTENT_MIN_SCORE;
import static com.example.ai.ragent.rag.constant.RAGConstant.MAX_INTENT_COUNT;
import static com.example.ai.ragent.rag.enums.IntentKind.SYSTEM;
...
public class IntentResolver {
    ...
    private List<SubQuestionIntent> capTotalIntents(List<SubQuestionIntent> subIntents) {
        ...
        // 步骤3：计算剩余配�?        int remaining = MAX_INTENT_COUNT - guaranteedIntents.size();
    ...
    private List<IntentCandidate> collectAllCandidates(List<SubQuestionIntent> subIntents) {
        ...
        // 按分数降序排�?        candidates.sort((a, b) -> Double.compare(b.nodeScore().getScore(), a.nodeScore().getScore()));
        return candidates;
    }
    ...
    private List<IntentCandidate> selectTopIntentPerSubQuestion(List<IntentCandidate> allCandidates, int subQuestionCount) {
        ...
        for (IntentCandidate candidate : allCandidates) {
            ...
            // 所有子问题都有了保底意图，提前退�?            if (topIntents.size() == subQuestionCount) {
                break;
            }
        }
        return topIntents;
    ...
    private List<IntentCandidate> selectAdditionalIntents(List<IntentCandidate> allCandidates,
        ...
        for (IntentCandidate candidate : allCandidates) {
            // 跳过已经被选为保底的意�?            if (guaranteedIntents.contains(candidate)) {
                continue;
            }
    ...
    private List<SubQuestionIntent> rebuildSubIntents(List<SubQuestionIntent> originalSubIntents,
        ...
        // 合并所有选中的意�?        List<IntentCandidate> allSelected = new ArrayList<>(guaranteedIntents);
