...
/**
 * 术语映射缓存管理�? * 负责术语映射规则�?Redis 中的缓存管理
 */
@Slf4j
@Component
...
public class QueryTermMappingCacheManager {
    ...
    public void saveMappingsToCache(List<QueryTermMappingDO> mappings) {
        try {
            ...
            log.info("术语映射已保存到 Redis 缓存，共 {} 条规�?, mappings.size());
        } catch (Exception e) {
            log.error("保存术语映射�?Redis 缓存失败", e);
        }
    }
