...
import com.example.ai.ragent.rag.controller.request.MessageFeedbackRequest;
...
import com.example.ai.ragent.rag.dao.mapper.MessageFeedbackMapper;
import com.example.ai.ragent.rag.mq.event.MessageFeedbackEvent;
...
public class MessageFeedbackServiceImpl implements MessageFeedbackService {
    ...
    public void submitFeedbackAsync(String messageId, MessageFeedbackRequest request) {
        ...
        Assert.notNull(vote, () -> new ClientException("反馈值不能为�?));
        Assert.isTrue(vote == 1 || vote == -1, () -> new ClientException("反馈值必须为 1 �?-1"));
    ...
    private ConversationMessageDO loadAssistantMessage(String messageId, String userId) {
        ...
        Assert.notNull(message, () -> new ClientException("消息不存�?));
    ...
    private void doUpsertFeedback(String messageId, String userId, String conversationId,
        ...
        if (existing == null) {
            ...
            // 仅当本次提交时间晚于记录最后更新时间时才覆盖，避免多节点并行消费乱�?            feedbackMapper.update(
    ...
    public void submitFeedbackByEvent(MessageFeedbackEvent event) {
        ...
        Assert.notNull(event.getVote(), () -> new ClientException("反馈值不能为�?));
        ConversationMessageDO message = loadAssistantMessage(messageId, userId);
        doUpsertFeedback(
                messageId,
                userId,
