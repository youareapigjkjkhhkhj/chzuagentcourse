...
import com.example.ai.ragent.rag.service.ratelimit.FairDistributedRateLimiter.AcquireRequest;
...
public class ChatQueueLimiter {
    ...
    private void handleReject(String question, String conversationId, SseEmitter emitter) {
        ...
        try {
            ...
            log.warn("记录 reject 会话失败，仍向前端发�?DONE", ex);
        }
    ...
    private RejectedContext recordRejectedConversation(String question, String conversationId, String userId) {
        ...
        if (StrUtil.isBlank(conversationId)) {
            // 入参未带 conversationId：刚生成的雪�?ID 不可能命中已有会话，跳过 existence 查询
        ...
        if (isNewConversation) {
            // append(USER) 内部会触�?conversationService.createOrUpdate（含 LLM 生成标题），此处回查拿到生成结果
