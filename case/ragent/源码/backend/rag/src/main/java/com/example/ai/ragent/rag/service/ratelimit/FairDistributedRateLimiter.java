...
public final class FairDistributedRateLimiter {
    ...
    /**
     * entry TTL �?maxWaitMillis 之上的额外缓冲，避免毫秒级时钟漂移导致存活条目被误判为僵�?     */
    ...
    public void start() {
        ...
        // 后续 acquire/availablePermits 不再重复调用，避免每�?poller 多一�?Redis 往�?        redissonClient.getPermitExpirableSemaphore(semaphoreKey).trySetPermits(maxPermitsSupplier.getAsInt());
    ...
    public void acquire(AcquireRequest req) {
        ...
        // entry 存活标记必须先于入队写入，否�?race 窗口内的并发 claim 会把刚入队的条目当僵�?ZREM
    ...
    /**
     * �?CAS 协调点。终态互斥：状态一旦从 PENDING 转走就不再变更，业务回调最多触发一�?     * 资源清理 ({@link Ticket#cleanup()}) 与状态机解耦，幂等执行
     */
    ...
    private final class Ticket {
         ...
         *
         * <p>GRANTED 状态下不释�?permit —�?permit 已经�?{@link #grant} 包装�?try/finally 接管�?         * 业务执行完毕（含异常）会�?finally 释放。这里跨界释放会导致并发请求拿到尚在使用�?permit
         */
        void cancel() {
        ...
        /**
         * 排队超时：CAS 抢占终态后 cleanup + �?caller executor 上跑 onTimeout
         */
        void timeout() {
        ...
        /**
         * 拿到 permit。CAS 抢占 GRANTED 终态后�?permit 生命周期交给业务（try/finally 释放�?         *
         * <p>permitRef 设值与 CAS 顺序：先 set，再 CAS。这样并�?cancel/timeout 路径�?CAS 失败
         * 时能看到 permit 并正确释放，避免 grant �?cancel 时序竞争导致 permit 泄漏
         */
        boolean grant(String permitId) {
            permitRef.set(permitId);
            if (!state.compareAndSet(State.PENDING, State.GRANTED)) {
                // 已被 cancel/timeout 抢占。permitRef 可能已被对方 cleanup 清空，CAS 防双重释�?                if (permitRef.compareAndSet(permitId, null)) {
            ...
            try {
                ...
                releaseHeldPermit();   // 业务未运行，必须显式释放（cleanup �?GRANTED 状态不会释�?permit�?                cleanup();
        ...
        /**
         * 释放当前 Ticket 持有�?permit（若有）。线程安�?+ 幂等
         */
        ...
        /**
         * 幂等清理：移队、删�?entry 标记、释�?permit（仅在非 GRANTED 状态下）、注销 poller、取�?future
         *
         * <p>GRANTED 状态下 permit 已由 grant 的包�?Runnable 接管，cleanup 不再释放�?         * 否则会在业务运行期间�?permit 还给 semaphore，等价于把同一 slot 让给另一个请�?         */
        void cleanup() {
    ...
    private boolean tryAcquireIfReady(Ticket ticket) {
        ...
        if (permitId == null) {
            // 队头但无 permit：按�?score 重入队，保留排队位次（公平性）
            // �?cancel/timeout �?race：claimIfReady �?ZREM，cleanup �?remove 在此刻是 no-op�?            // 必须 add 后回�?state，若已终态则自行回滚，避免僵尸条目永久占据队头窗�?            setEntryMarker(ticket.requestId, Math.max(1, ticket.deadline - System.currentTimeMillis()));
            RScoredSortedSet<String> queue = redissonClient.getScoredSortedSet(queueKey, StringCodec.INSTANCE);
        ...
        if (!ticket.isPending()) {
            // claim �?acquire 之间被取�?超时：必须释�?permit 并通知，否则其他等待者要等下一�?poll
            releasePermitQuietly(permitId);
            publishQueueNotify();
            return false;
        }
    ...
    /**
     * 写入 entry 存活标记，TTL = 等待预算 + 缓冲。JVM 崩溃�?Key 自然过期�?     * 后续 {@link #claimIfReady} �?Lua 内会把对�?ZSet 条目当僵尸清理掉，避免永久占据队头窗�?     */
    ...
    /**
     * @return 成功返回 ticket 的原�?score（用于失败时按原位次重入队），未 claim 返回 -1
     */
    ...
    /**
     * 跨实�?RTopic 通知到达后，批量唤醒本进程所�?poller
     *
     * <p>通过 {@code firing} CAS + {@code pendingNotifications} 计数做合并：连续到达的多次通知
     * 只触发一次扫描，避免风暴。复用外�?scheduler 执行扫描，无需独立线程
     */
                ...
                do {
                    ...
                    try {
                        if (permitSupplier.getAsInt() <= 0) {
                            // permit 已耗尽，本轮不必扫描所�?poller。下一次真正的 release 会发新通知重新驱动
                            break;
                        }
