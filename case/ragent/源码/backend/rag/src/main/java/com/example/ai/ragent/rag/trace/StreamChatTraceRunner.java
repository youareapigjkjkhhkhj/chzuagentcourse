...
package com.example.ai.ragent.rag.trace;
...
import com.example.ai.ragent.infra.chat.ForwardingStreamCallback;
...
public class StreamChatTraceRunner {
    ...
    /**
     * @param businessLogic 接收 trace 增强后的 callback：onComplete / onError 会触�?finishRun
     *                      实现方负责把�?callback 注入�?pipeline 上下文中并触�?pipeline 执行
     */
    public void run(String question,
        ...
        try {
            ...
            // �?traceAwareCallback.onError 以复用其内部 CAS，避免与 pipeline 内已触发的终态重复收�?            try {
                traceAwareCallback.onError(ex);
            } catch (Throwable ignored) {
                // 即使 callback.onError 失败也要保证 trace 能收�?            }
        } finally {
            // 同步阶段结束就清�?ThreadLocal，避免污染线程池中复用的线程�?            // 异步线程通过 TTL 已拿�?traceId 的快照副本，不依赖此线程�?ThreadLocal
