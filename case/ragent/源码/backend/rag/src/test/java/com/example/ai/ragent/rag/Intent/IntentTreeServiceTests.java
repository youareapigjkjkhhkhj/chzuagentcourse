...
public class IntentTreeServiceTests {
    ...
    public void getFullTree() {
        ...
        if (roots == null || roots.isEmpty()) {
            System.out.println("(意图树为�?");
            return;
        }

        // 顶层先按 sortOrder 排一下，避免顺序�?        roots = roots.stream()
    ...
    private void printNodeTree(IntentNodeTreeVO node, int depth, StringBuilder sb) {
        ...
        // 2) 备注（description�?        if (node.getDescription() != null && !node.getDescription().isBlank()) {
            sb.append(indent)
                    .append("  ")
                    .append("备注: ")
        ...
        // 3) 示例问题（examples�?        if (node.getExamples() != null && !node.getExamples().isEmpty()) {
        ...
        // 4) 子节�?        if (node.getChildren() != null && !node.getChildren().isEmpty()) {
