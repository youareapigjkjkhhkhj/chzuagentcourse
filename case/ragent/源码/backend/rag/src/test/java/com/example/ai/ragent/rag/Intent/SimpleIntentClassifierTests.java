...
package com.example.ai.ragent.rag.Intent;
...
public class SimpleIntentClassifierTests {
    ...
    private String buildIntentPrompt(String question) {
        return """
                你是一个企业内部知识库 RAG 系统中的【意图识别模块】�?                你的任务是：根据用户提出的问题，判断该问题属于哪一类业务意图，
                并给出推荐的集合名称和元数据过滤条件，以便向量数据库检索�?                
                请特别注意：
                1. 需要支持【模糊问法和同义词】，例如�?                   - “上班时间”“几点上班”“每天要工作多久�?�?归类�?ATTENDANCE（考勤/工作时间�?                   - “能不能多休几天”“休假怎么安排�?�?ATTENDANCE
                ...
                2. 用户问题可能同时涉及多个内容时，选择【对用户当前核心问题最重要】的那个意图作为主意图�?                3. 如果无法明显归类，使�?GENERAL�?                
                ...
                同时，你需要给出：
                - 对应的推荐向量集合（collectionName），例如�?                  - 大部分人�?制度类问题：rag_hr_collection
                  - IT 支持类问题：rag_it_collection
                - 可选的元数据过滤条件（filterExpr），用于 Milvus expr�?                  例如：biz_type in ["ATTENDANCE","WORKING_HOURS"]
                
                【输出格式要求（非常重要）】：
                1. 只输出一�?JSON 对象，不要包含任何多余文字�?                2. JSON 字段包括�?                   - intent: 字符串，取值为 ["RECRUITMENT","COMPENSATION","ATTENDANCE","POLICY","IT_SUPPORT","GENERAL"] 之一
                   - collectionName: 字符串，向量集合名称，如 "rag_hr_collection" �?"rag_it_collection"
                   - filterExpr: 字符串或 null，Milvus �?expr 过滤条件，如 "biz_type in [\\"ATTENDANCE\\"]"
                   - confidence: 0 �?1 之间的小数，表示你对该分类的信心�?                   - reason: 字符串，简要说明你这样分类的理�?                
                示例输出�?                {
                  "intent": "ATTENDANCE",
                  "collectionName": "rag_hr_collection",
                  "filterExpr": "biz_type in [\\"ATTENDANCE\\",\\"WORKING_HOURS\\"]",
                  "confidence": 0.92,
                  "reason": "问题询问工作时间和加班制度，属于考勤与工作时间相�?
                }
    ...
    private CategoryScore scoreCategory(Category category, String question) {
        ...
        // 期望返回�?        // {"score":0.95,"reason":"问题�?Mac 打印机连接，属于 IT 支持场景"}
        try {
            ...
            // 解析失败�?0 �?            return new CategoryScore(category, 0.0, "parse_failed", resp);
        }
    }
    ...
    private String buildCategoryScorePrompt(Category category, String question) {
        ...
        return """
                你是一个企业内部知识库 RAG 系统中的【意图评分模块】�?                现在你只负责判断：当前这个“用户问题”，和下面这个【单一分类】是否属于同一类问题�?                
                请根据分类的说明和典型问题示例，判断该用户问题与该分类的相关度，并打一�?0~1 的分数�?                
                评分说明�?                - 0.0：完全无�?                - 0.3：略有关系，但大概率不是这个分类
                - 0.6：有一定关系，可以考虑作为候�?                - 0.8~1.0：高度相关，几乎可以认为就是这个分类的问�?                
                请注意：
                - 需要支持模糊问法和同义表达（例如“上班时间”“几点打卡”“每天工作多久”，都归入考勤/工作时间类）�?                - 不需要和其他分类比较，只评估「当前分�?vs 当前问题」这一对的相关度�?                
                【当前分类�?                编码�?s
                说明�?s
                
                【该分类的典型问题示例（仅示例，并不完整）�?                %s
                
                【用户问题�?                %s
                
                【输出格式要求】：
                1. 只输出一�?JSON对象，不要包含任何多余文字�?                2. 必须包含字段�?                   - score: 0~1 的小数，例如 0.85
                   - reason: 简要中文说明你打这个分的原�?                
                示例输出�?                {
                  "score": 0.93,
                  "reason": "问题是关�?Mac 打印机连接，�?IT 支持类非常接�?
                }
                """
                .formatted(
                        category.code(),
