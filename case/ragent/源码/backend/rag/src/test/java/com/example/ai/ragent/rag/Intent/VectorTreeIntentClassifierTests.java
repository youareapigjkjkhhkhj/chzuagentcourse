...
public class VectorTreeIntentClassifierTests {
    ...
    /**
     * 场景 1：考勤 + 处罚 混合语义
     * 期望：能命中某个人事实体节点（比如“人�?考勤制度”之类的叶子节点）�?     */
    @Test
    ...
    private void runCase(String question) {
        // 你可以根据实际情况调这两个参�?        double MIN_SCORE = 0.35; // 低于这个就认为“不太像”，可以不检�?        int TOP_N = 5;           // 最多只看前 5 个候�?
        ...
        if (allScores.stream().noneMatch(ns -> ns.score() >= MIN_SCORE)) {
            System.out.println("  (no target above threshold, 可以考虑不走向量检索或�?fallback)");
        }
