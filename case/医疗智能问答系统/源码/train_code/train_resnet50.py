...
def create_model(model_name, pretrained_path=None, num_classes=5):
    ...
    if pretrained_path and os.path.exists(pretrained_path):
        ...
        # 关键：ResNet 系列的分类头叫 'fc'，不是 'classifier'
        if 'fc.weight' in state_dict:
            print("Removing original fc layer (1000 classes)")
            state_dict.pop('fc.weight')
            state_dict.pop('fc.bias')
        # EfficientNet 系列才叫 classifier
        elif 'classifier.weight' in state_dict:
            state_dict.pop('classifier.weight')
            state_dict.pop('classifier.bias')
        
        missing_keys, unexpected_keys = model.load_state_dict(state_dict, strict=False)
        print(f"Backbone loaded successfully. Missing: {len(missing_keys)} keys (normal for new fc)")
...
def main(model_name):
    ...
    os.makedirs("figures", exist_ok=True)  # 保存图像目录
    ...
    for epoch in range(1, CFG.epochs + 1):
        ...
        plt.tight_layout()
        plt.savefig(f"figures/train_val_epoch_{epoch}.png")
        plt.close()
    ...
    plt.tight_layout()
    plt.savefig("figures/final_train_val_curve.png")
    plt.close()

    print("Training done. All figures saved in 'figures/'")
