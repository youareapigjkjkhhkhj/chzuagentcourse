...
def create_model(model_name, pretrained_path=None, num_classes=5):
    ...
    if pretrained_path and os.path.exists(pretrained_path):
        ...
        # 万能删除分类头（兼容所有 timm 模型）
        keys_to_remove = []
        for k in state_dict.keys():
            if k.startswith(('head.', 'classifier.', 'fc.')) or k in ['head.weight', 'head.bias', 'classifier.weight', 'classifier.bias', 'fc.weight', 'fc.bias']:
                keys_to_remove.append(k)
        
        for k in keys_to_remove:
            print(f"Removing mismatched classification head: {k}")
            state_dict.pop(k, None)
        
        # 加载剩余权重
        missing_keys, unexpected_keys = model.load_state_dict(state_dict, strict=False)
        print(f"Backbone loaded! Missing {len(missing_keys)} keys (normal for new head)")
